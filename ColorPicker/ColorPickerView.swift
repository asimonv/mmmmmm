...
struct ColorPickerView: View {
    ...
    private var currentColor: Color {
        Color(UIColor.init(hue: self.normalizeGesture().y / linearGradientHeight, saturation: 1.0, brightness: 1.0 - self.normalizeGesture().x / 320, alpha: 1.0))
    }
    ...
    private func normalizeGesture() -> (x: CGFloat, y: CGFloat) {
        let offsetY = startLocation + dragOffset.height // Using our starting point, see how far we've dragged +- from there
        ...
        let offsetX = startLocation + dragOffset.width
        let maxX = max(0, offsetX)
        let minX = min(maxX, 320)
                ...
                .gesture(
                    DragGesture()
                        .onChanged({ (value) in
                            ...
                            self.chosenColor = UIColor.init(hue: self.normalizeGesture().y / self.linearGradientHeight, saturation: 1.0, brightness: 1.0 - self.normalizeGesture().x / 320, alpha: 1.0)
